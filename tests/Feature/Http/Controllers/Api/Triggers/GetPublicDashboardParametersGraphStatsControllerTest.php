...
it('return expected parameters stats by parameter', function () use ($visits) {
    $dashboard = dashboard(['public' => true]);
    ...
    actingAs($user)
        ->getJson('/api/dashboards/'.$dashboard->uuid.'/triggers/'.$trigger->uuid.'/parameters-graph-stats/referrer?date=2023-06-08')
        ->assertJson(fn (AssertableJson $json) => $json
            ->where('data.period.date', '2023-06-09T00:00:00+00:00')
            ->where('data.period.period', '30d')
            ->has('data.plot.referrer')
