...
<div class="relative">
    ...
    <div class="p-app mt-12 {{!$withoutTitle ? 'sm:mt-44' : ''}} mw-landing mx-auto">
        ...
        <div class="mw-landing mx-auto flex flex-col space-y-6 sm:flex-row sm:space-y-0 sm:space-x-6">
            @foreach(app(GetLandingPricesService::class)()->prices as $price)
                <a
                    {!! $price->remaining === 0 ? '' : ' href="/leads/create/prices/' . $price->id . '?utm_source=metricswave&utm_medium=referral&utm_campaign=pricing"' !!}
                    class="backdrop-blur-x duration-400 flex-1 rounded border bg-white/50 p-6 transition-all
                dark:border-zinc-600 dark:bg-zinc-700/50
                dark:shadow-zinc-400 {{ $price->remaining > 0 ? 'shadow hover:bg-white/90 dark:hover:bg-zinc-700' : 'cursor-default opacity-60' }}
                ">
                    ...
                    @php
                        ...
                        $color = match (true) {
                            ...
                            $price->remaining !== 0 && $percentage < 0.25 => 'text-orange-500',
                    ...
                    <div class="mt-6 w-full text-center">
                        @if($price->remaining <= 0)
                            <div
                                class="duration-400 mx-auto block cursor-default rounded border bg-white/30 py-4 text-center backdrop-blur-none transition-all dark:border-zinc-50/30 dark:bg-white/10 dark:shadow-zinc-500/50">
                                Sold Out
                            </div>
                        @else
                            <div
